...
class HeatmapGenerator(IGenerator):
    """
    Generates a synthetic workload based on a percentage-based heatmap,
    learning localized patterns for operation frequency, target popularity
    per command, and inter-arrival rhythms for each time interval.
    """
    def __init__(
        self,
        parser: IParser,
    ...
    def _characterize(self, events: List[FEIEvent]) -> Dict[str, Any]:
        ...
        if self.auto_duration:
            self.simulation_duration_ms = float(total_duration_ms)
            self.simulation_duration_s = float(total_duration_ms) / 1000.0
            print(f"Auto-duration enabled: Simulation set to exactly {self.simulation_duration_s:.3f} seconds.")
        ...
        opt_heatmap = {
            ...
            for k, v in heatmap_probabilities.items()
        }

        opt_target = defaultdict(dict)
        for interval_idx, op_data in target_probabilities.items():
            for op_type, targets in op_data.items():
                opt_target[interval_idx][op_type] = (list(targets.keys()), list(targets.values()))

        opt_inter_arrival = {
            k: (list(cnts.keys()), list(cnts.values()))
            for k, cnts in inter_arrival_probabilities.items()
        }
        ...
        return {
            ...
            "heatmap": opt_heatmap,
            "target_probabilities_by_op": opt_target,
            "inter_arrival_probabilities": opt_inter_arrival,
    ...
    def _synthesize(self, model: Dict[str, Any]) -> List[FEIEvent]:
        ...
        available_pool: Set[str] = set(model.get('initial_resource_pool', []))
        ...
        while current_time_ms < simulation_duration_ms:
            ...
            op_keys, op_weights = action_dist
            op_type = random.choices(op_keys, op_weights)[0]
            ...
            target_keys, target_weights = target_dist
            target = random.choices(target_keys, target_weights)[0]
            ...
            delta_keys, delta_weights = delta_dist
            delta_ms_original = random.choices(delta_keys, delta_weights)[0]

            event_timestamp_ms = current_time_ms
            ...
            if is_create_update:
                ...
                if not available_pool or target not in available_pool:
            ...
            new_raw_args = self.parser.generate_args(op_type, target, available_pool=available_pool)

            synthetic_events.append(FEIEvent(
                timestamp=(float(event_timestamp_ms) / 1000.0),
